"""
Fixed Synthetic Data Generation for Smart Irrigation System
- Wider soil moisture range (15-85%)
- More realistic rainfall events
- Better seasonal variation
- Proper feature alignment with backend
"""
...
class RainGenerator:
    ...
    def generate(self, season):
        ...
        if np.random.rand() < prob:
            self.rain_memory = np.random.randint(1, 4)
            if season == "monsoon":
...
if __name__ == "__main__":
    ...
    # Save to Code/backend/data for backend to use
    df.to_csv("../backend/data/sensor_readings.csv", index=False)
    print(f"✓ Dataset generated: {df.shape}")
    print(f"  Soil moisture range: {df['soil_moisture'].min():.1f}% - {df['soil_moisture'].max():.1f}%")
    print(f"  Rainfall events: {(df['rain'] > 0).sum()} / {len(df)}")
    print(f"  Irrigation events: {(df['irrigation'] > 0).sum()} / {len(df)}")
